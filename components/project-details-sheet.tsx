...
function ListingDesktopColumn({
  ...
  return (
    <div
      className={`flex min-w-0 flex-col items-center px-2 text-center xl:px-3 ${className ?? ""}`}
    >
      <span
        className="mb-2 inline-flex size-6 shrink-0 items-center justify-center text-[#c4a574]"
        aria-hidden
      >
        {icon}
      </span>
      <p className="text-xs font-medium leading-tight text-[#c4a574] xl:text-sm">
        {label}
      </p>
      <div className="mt-2 text-pretty text-[0.85rem] font-medium leading-snug text-[var(--color-text)] xl:text-[0.92rem]">
        {children}
      </div>
    </div>
  );
    ...
    <>
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2">
          ...
          <ListingMobileColumn label={t("detailLabelType")} icon={typeIcon}>
      ...
      <div
        dir={dir}
        lang={locale === "he" ? "he" : "en"}
        className={`project-details-sheet hidden lg:flex h-full min-h-0 flex-col text-center ${desktopCardClassName} px-5 py-5 xl:px-6 xl:py-6`}
      >
        <div className="shrink-0">
          <h2 className="text-[1.35rem] font-medium tracking-tight text-[var(--color-text)] xl:text-[1.5rem]">
            {t("detailSectionTitle")}
          </h2>
          <div
            className="mx-auto mt-4 h-px w-full bg-[linear-gradient(to_left,color-mix(in_oklab,#c4a574_74%,transparent),transparent)]"
            aria-hidden
          />
          <div className="mt-4 grid grid-cols-2 xl:mt-5">
            <ListingDesktopColumn
              label={t("detailLabelClient")}
              icon={clientIcon}
              className="border-inline-end border-[color-mix(in_oklab,#c4a574_34%,transparent)]"
            >
              <h2
                className="m-0 font-semibold text-[inherit]"
                id={`project-heading-${slug}`}
              >
                {title}
              </h2>
            </ListingDesktopColumn>
            <ListingDesktopColumn label={t("detailLabelType")} icon={typeIcon}>
              <span className="font-semibold">{cardSubtitle}</span>
            </ListingDesktopColumn>
          </div>
        </div>
        ...
        {paragraphs.length > 0 ? (
          <div className="flex min-h-0 flex-1 flex-col justify-center py-3 xl:py-4">
            <div className="flex flex-col items-center px-1">
              <p className="text-xs font-medium text-[#c4a574] xl:text-sm">
                {t("detailLabelAbout")}
              </p>
              <div
                className="mt-2 h-px w-12 bg-[linear-gradient(to_left,color-mix(in_oklab,#c4a574_74%,transparent),transparent)]"
                aria-hidden
              />
              <div className="mt-3 max-w-full space-y-2.5 text-pretty text-center text-[0.84rem] font-normal leading-[1.7] text-[var(--color-text)]/85 xl:text-[0.9rem] xl:leading-[1.75]">
                {paragraphs.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
              </div>
        ...
        {onOpenGallery ? (
          <div className="flex shrink-0 justify-center border-t border-[color-mix(in_oklab,#c4a574_28%,transparent)] pt-4 xl:pt-5">
            <button
              type="button"
              onClick={onOpenGallery}
